		...
		<div class="wrap">
			...
			<div class="detail">
				<div class="store">
					...
					<div class="content">
						<div class="desc">
							<div class="img">
								<img onerror="this.src='/resources/assets/images/noimage.png'" src="/display?uuid=${store.attach.uuid}&path=${store.attach.path}&origin=${store.attach.origin}">
							</div>
							<div class="con">
								<div class="stars">
									<div class="star">
			...
			<form method="post" id="frm">
				<div class="order">
					...
					<div class="button">
						<button type="button" class="btn btnOrder" id="btnOrder">주문하기</button>
					</div>
				</div>
	...
	<script type="text/javascript">
        ...
        $(function() {
			...
			function getMenuStr(menu) {
				...
				if(menu.attach) {
					str += '	<div class="img">';
					str += '		<img src="/display?uuid=' + menu.attach.uuid + '&path=' + menu.attach.path + '&origin=' + menu.attach.origin + '">';
					str += '	</div>';
				}
				str += '</div>';
            ...
            function getCartStr(cart) {
				var str = '';
				str += '<div class="desc removeCart" data-cno="' + cart.cno + '" data-mno="' + cart.mno + '" data-count="' + cart.count + '" data-id="' + cart.id + '" data-price="' + cart.price + '" data-sno="' + cart.sno + '" data-name="' + cart.name + '">';
				str += '	<div class="con">';
				str += '		<span class="txt">' + cart.name  + '</span>';
				str += '		<span class="float-right mx-2 btnReplyRemove"><i class="fas fa-times"></i></span>';
				str += '	</div>';
				str += '	<div class="pri">';
				str += '		<div class="cart_count">';
				str += '			<button type="button" class="quantity_minus" id="minus_cart"></button>';
				str += '			<span class="quantity_number" id="numberPlace">&nbsp;' + cart.count + '&nbsp;</span>';
				str += '			<button type="button" class="quantity_plus" id="plus_cart"></button>';
				str += '			<span class="float-right price">' + cart.price + '원</span>';
				str += '		</div>';
				str += '	</div>';
				str += '</div>';
			...
			function getCartTotalStr(){
				var str = '';
				
				str += '<div class="desc removeCart text-align-end">';
				str += '	<div>배달요금 ${store.delPrice}원 별도</div>';
				str += '	<div>총 <input type="text" class="bg-white inputTotalPrice" value="" name="inputTotalPrice"  readonly>원</div>';
				str += '</div>';
			...
			function showCartList() {
				...
				cartService.getList(function(result) {
					...
					price += ${store.delPrice};											
					str += getCartTotalStr();
					$(".cartList").html(str);					
					$('input[name=inputTotalPrice]').attr('value', price);
            	}, function(xhr) {
            		//console.log(xhr);
            	})
			...
			// 카트 수량 추가
			$(".cartList").on("click", "#plus_cart", function(){
				event.preventDefault(); // 기본 이벤트 제거
				...
				cartService.plus(cno, function(result){
					console.log(result)
					showCartList()
					alert("더하기 성공")
				})
			...
			// 카트 수량 삭제
			$(".cartList").on("click", "#minus_cart", function(){
				event.preventDefault(); // 기본 이벤트 제거
				
				var cno = $(this).closest(".removeCart").data("cno");
				var count = $(this).closest(".removeCart").data("count");
				...
				if(count == 1) {
					...
					cartService.minus(cno, function(result){
						console.log(result)
						showCartList()
						alert("빼기 성공")
					})					
			...
			$("#btnOrder").on("click", function(e){
				console.log("으아아아아");
				//$(".cartList div:first-child").data();
				
				//console.log($(".cartList div:nth-child(1)").data());
				var attrArr = ['cno', 'count', 'id', 'mno', 'name', 'price', 'sno'];
				var str = "";
				$(".cartList > div[data-cno]").each(function(i) {
					for(var j in attrArr) {
						str += 
							$("<input>")
							.attr("type", "hidden")
							.attr("name", "orderMenus[" + i + "]." + attrArr[j])
